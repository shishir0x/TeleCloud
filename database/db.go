...
func EnsureFoldersExist(dbPath string, owner string) error {
	...
	for i := 1; i < len(parts); i++ {
		...
		if err != nil {
			...
			if count == 0 {
				_, err = DB.Exec(InsertIgnoreSQL("files", "filename, path, is_folder, owner", "?, ?, TRUE, ?"), part, currentPath, owner)
				if err != nil {
					return err
				}
			}
...
func EnsureFoldersExistTx(tx *WrappedTx, dbPath string, owner string) error {
	...
	for i := 1; i < len(parts); i++ {
		...
		if err != nil {
			...
			if count == 0 {
				_, err = tx.Exec(InsertIgnoreSQL("files", "filename, path, is_folder, owner", "?, ?, TRUE, ?"), part, currentPath, owner)
				if err != nil {
					return err
				}
			}
